...
if [[ -n "$DEVBOX_UID" && -n "$DEVBOX_GID" ]];then
    ...
    usermod -u $DEVBOX_UID mysql
    groupmod -g $DEVBOX_GID mysql

fi

#########################################################################################################################################
###
## refer https://raw.githubusercontent.com/percona/percona-docker/5e8402df063be2a8174bedff478391dbc881ff2b/percona-server-5.7/ps-entry.sh
